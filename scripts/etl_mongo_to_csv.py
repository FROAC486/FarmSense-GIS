...
def build_geojson(lon, lat, alt=None):
...
def extract_tags_location(device_info):
    ...
    if lat is None or lon is None:
        if tag_lat is not None and tag_lon is not None:
            ...
            lon = tag_lon
            alt = tag_alt
            geojson_clean = build_geojson(lon, lat, alt)
...
def extract_gateway_info(doc):
    ...
    return {
        ...
        "gateway_geojson": build_geojson(gateway_lon, gateway_lat, gateway_alt),
        "rssi": rssi,
        "snr": snr
    }
...
def flatten_doc(doc):
    ...
    tag_location = extract_tags_location(device_info)
    gateway_info = extract_gateway_info(doc)
    environmental = extract_environmental_fields(obj, device_profile_name)
    tank = extract_tank_fields(obj, device_profile_name)
    sensecap = extract_sensecap_fields(obj, device_profile_name)
    ...
    row.update(tag_location)
    row.update(gateway_info)
    row.update(environmental)
    row.update(tank)
    row.update(sensecap)
...
if __name__ == "__main__":
    try:
        ...
        print("ETL failed:", e)
# USE ME ---> & ".\.venv\Scripts\python.exe" etl_mongo_to_csv.py